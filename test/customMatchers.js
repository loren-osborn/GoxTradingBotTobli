        ...
        jasmine.iterateOverTestDataSets = (function iterateOverTestDataSets(dataSets, paramOverrides, testCallback) {
            ...
            for (i = 0; i < dataSets.length; i++) {
                ...
                if (paramOverrides[dataSets[i].name] !== undefined) {
                    ...
                    dataSets[i].data = paramOverrides[dataSets[i].name];
                }
                totalIterations *= dataSets[i].data.length;
            }
            for (i = 0; i < totalIterations; i++) {
                prameters = [];
                remainder = i;
                for (j = (dataSets.length - 1); j >= 0; j--) {
                    paramIndex = (remainder % (dataSets[j].data.length));
                    remainder = (remainder - paramIndex) / (dataSets[j].data.length);
                    prameters.unshift(dataSets[j].data[paramIndex]);
                }
